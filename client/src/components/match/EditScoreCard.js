...
const {team1, team2, id, scoreCard=[]} = location?.state;
  ...
  useEffect(()=>{
    getPlayers(team1.teamId).then(players=>{
        // players.map(p=> {
        //     let existingScore = scoreCard.find(score=> score.player.id ===p.id);
        //     if(existingScore){
        //         return {
        //             scoreCardId: existingScore?.id? existingScore.id: 0, player: existingScore.player, score: existingScore.score
        //         };
        //     }
        //     return {
        //         scoreCardId:  0, player: p, score: null
        //     }
        // })
...
const savePlayerScores = (e)=>{
  ...
  return (
    <form className={classes.root} noValidate autoComplete="off" onSubmit={savePlayerScores}>
      <div className={classes.team1}>
          <h1>{team1.teamName}</h1>
          {team1Players && team1Players.map(p => (
              
      <TextField id={`player_${p.id}`} type="number" label={`${p.firstName} ${p.lastName} Score`} defaultValue={getPlayerScore(p.id)} />
          ))}
          </div>  
          ...
          {team2Players && team2Players.map(p => (
              ...
              <TextField id={`player_${p.id}`} label={`${p.firstName} ${p.lastName} Score`} defaultValue={getPlayerScore(p.id)} />
                  ))}
          </div>
